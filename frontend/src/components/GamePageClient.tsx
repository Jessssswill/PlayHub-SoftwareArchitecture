...
export default function GamePageClient({ sessionId }: Props) {
  ...
  const submitMove = useCallback(
    async (moveData: Record<string, unknown>) => {
      ...
      try {
        ...
        const msg =
          axiosMsg ?? (e instanceof Error ? e.message : "Move failed");
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      {!myPlayerId && (
        <div
          className={`mb-6 p-4 ${card} border-zinc-300 dark:border-zinc-700`}
        >
          <p className={`text-sm font-medium ${tokens.text} mb-3`}>
            Who are you?
          </p>
          ...
          <div className={`${card} text-sm space-y-1`}>
            ...
            {myPlayerId && myPlayerId !== "spectator" && (
              <p className={tokens.textMuted}>
                You:{" "}
                {currentSession.players.find((p) => p.id === myPlayerId)?.name}
              </p>
            )}
          </div>
        </div>
