...
export default function ChessBoard({ board, onMove, disabled, myPlayerId, currentPlayerId, playerOrder }: Props) {
  ...
  return (
    <div className={`inline-block border-2 ${tokens.border} rounded overflow-hidden`}>
      {board.map((row, r) => (
        <div key={r} className="flex">
          <span className={`w-6 flex items-center justify-center text-xs ${tokens.textMuted} select-none ${tokens.bg}`}>
            {8 - r}
          </span>
          {row.map((cell, c) => {
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isLight ? 'bg-zinc-100 dark:bg-zinc-200' : 'bg-zinc-400 dark:bg-zinc-500',
      ...
      <div className={`flex ml-6 ${tokens.bg}`}>
        {['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'].map((f) => (
          <span key={f} className={`w-10 text-center text-xs ${tokens.textMuted} select-none py-0.5`}>{f}</span>
        ))}
      </div>
    </div>
  );
