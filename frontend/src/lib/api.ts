...
export const getSessions = (status?: string): Promise<GameSession[]> =>
  http
    .get<GameSession[]>("/sessions", { params: status ? { status } : {} })
    .then((r) => r.data);
...
export const createSession = (
  ...
  http
    .post<GameSession>("/sessions", { gameType, players })
    .then((r) => r.data);

export const joinSession = (id: string, playerId: string, playerName: string) =>
  http
    .post(`/sessions/${id}/join`, { playerId, playerName })
    .then((r) => r.data);
...
export const makeMove = (
  ...
  http
    .post<TurnResult>(`/sessions/${id}/move`, { playerId, move })
    .then((r) => r.data);

export const endSession = (id: string, requesterId: string) =>
  http
    .delete(`/sessions/${id}`, { params: { requesterId } })
    .then((r) => r.data);
