...
export class ChessRules {
  static validate(state: GameState, move: ChessMove): void {
    ...
    if (
      !ChessRules.inBounds(from.row, from.col) ||
      !ChessRules.inBounds(to.row, to.col)
    ) {
    ...
    if (!piece) {
      throw new BadRequestException(
        `Tidak ada piece di (${from.row},${from.col}).`,
      );
    }
    ...
    if (isWhiteTurn !== isWhitePiece) {
      throw new BadRequestException(
        'Kamu tidak bisa menggerakkan piece lawan.',
      );
    }
  ...
  static apply(state: GameState, move: ChessMove): GameState {
    ...
    newState.boardState[to.row][to.col] =
  ...
  static checkEnd(state: GameState): EndCondition {
    ...
    if (!whiteKing) {
      return {
        isOver: true,
        winnerId: state.playerOrder[1] ?? null,
        isDraw: false,
      };
    }
    if (!blackKing) {
      return {
        isOver: true,
        winnerId: state.playerOrder[0] ?? null,
        isDraw: false,
      };
    }
  ...
  private static isValidPieceMove(
    ...
    switch (piece) {
      case 'P':
        return ChessRules.pawn(state, move, isWhite);
      case 'R':
        return ChessRules.rook(state, move);
      case 'N':
        return ChessRules.knight(move);
      case 'B':
        return ChessRules.bishop(state, move);
      case 'Q':
        return ChessRules.rook(state, move) || ChessRules.bishop(state, move);
      case 'K':
        return ChessRules.king(move);
      default:
        return false;
    }
  }
