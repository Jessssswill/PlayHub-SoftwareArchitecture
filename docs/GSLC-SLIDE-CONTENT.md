...
| Pattern | Implementasi | Role dalam Proyek |
|---------|-------------|------------------|
| **Facade** | `GameEngineFacade` | Satu API untuk semua operasi game, menyembunyikan koordinasi 6 subsystem dari controller |
...
| **Proxy (Cache)** | `CachedGameStateProxy` | Cache getState() 1 detik + auto-invalidate via event, mengurangi registry lookup berulang |
